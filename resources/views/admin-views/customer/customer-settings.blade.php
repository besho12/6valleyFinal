        ...
        <form action="{{ route('admin.customer.customer-settings') }}" method="post" id="update-settings">
            ...
            <div class="card mb-3">
                <div class="border-bottom px-4 py-3">
                    <h5 class="mb-0 text-capitalize d-flex align-items-center gap-2">
                        <i class="tio-award"></i>
                        {{translate('customer_Ads_Point_Settings')}}
                        <span class="input-label-secondary cursor-pointer" data-toggle="tooltip" data-placement="right"
                              title="{{translate('in_this_settings_admin_can_set_the_rules_for_the_customers_for_earning_and_use_the_ads_points')}}">
                            <img width="16" src="{{dynamicAsset(path: 'public/assets/back-end/img/info-circle.svg')}}" alt="">
                        </span>
                    </h5>
                </div>
                <div class="card-body">
                    <div class="row align-items-end gy-2">
                        @php($adsPointExchangeRate=getWebConfig(name:'ads_point_exchange_rate'))
                        <div class="col-xl-4 col-md-6">
                            <div class="">
                                <label class="title-color" for="ads_point_exchange_rate">
                                    {{translate('equivalent_Point_to_1_Unit_Currency')}}
                                </label>
                                <input type="text" class="form-control" name="ads_point_exchange_rate"
                                       id="ads_point_exchange_rate"
                                       placeholder="{{translate('ex').':'.'10'}}"
                                       value="{{ $adsPointExchangeRate ?? 0 }}">
                            </div>
                        </div>
                        @php($adsPointMinimumPoint=getWebConfig(name:'ads_point_minimum_point'))
                        <div class="col-xl-4 col-md-6">
                            <div class="">
                                <label class="title-color" for="minimum_transfer_point">
                                    {{translate('minimum_Point_Required_To_Convert')}}
                                ...
                                <input type="text" class="form-control" name="ads_minimun_transfer_point"
                                       id="minimum_transfer_point"
                                       placeholder="{{translate('ex').':'.'10'}}"
                                       value="{{ $adsPointMinimumPoint ?? 0 }}">
                            </div>
                        </div>
                    </div>
                </div>
