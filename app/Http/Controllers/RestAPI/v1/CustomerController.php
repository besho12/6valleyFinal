...
class CustomerController extends Controller
{
    ...
    {
        $exist_user_points = AdsPoints::where('user_id', $request['user_id'])->first();

        if(!$exist_user_points){
            AdsPoints::create([
            ...
            $newPoints = floatval($exist_user_points['points']) + floatval($request['points']);
            AdsPoints::where('user_id', $request['user_id'])->update(['points'=>$newPoints]);
        }
        return response()->json(['message' => 'Successfully Added'], 200);
    }

    public function get_ads_points(Request $request)
    {
