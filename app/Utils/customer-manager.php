...
use App\Models\AdsPointsTransaction;
...
class CustomerManager
{
    ...
    public static function create_wallet_transaction($user_id, float $amount, $transaction_type, $referance, $payment_data=[])
    {
        ...
        if(in_array($transaction_type, ['add_fund_by_admin','add_fund','order_refund','loyalty_point','ads_point']))
        {
            $credit = $amount;
        ...
        try{
            ...
            DB::commit();
            if(in_array($transaction_type, ['loyalty_point','order_place','add_fund_by_admin','add_fund','ads_point'])) return $wallet_transaction;
            return true;
        }catch(\Exception $ex)
        {
            info($ex);
    ...
    public static function create_ads_point_transaction($user_id, $referance, $amount, $transaction_type)
    {
        ...
        $ads_point_transaction->transaction_type = $transaction_type;
        ...
        $current_balance = $user->loyalty_point + $credit - $debit;
        $ads_point_transaction->balance = $current_balance;
        $ads_point_transaction->credit = $credit;
        $ads_point_transaction->debit = $debit;
        $ads_point_transaction->created_at = now();
